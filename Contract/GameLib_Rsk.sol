...
/*  enum ResourceIndex {
    ...
    function getWarehouseLoadByLevel(uint level)
        ...
        returns(uint)
    {
        uint24[5] memory warehouseStorage = [1000, 5000, 15000, 130000, 1600000];
    ...
    {
        uint24[5] memory buildingCost = [0,529,2616,12096,55916];
        /*
    ...
    function getUpgradeResourceCost(uint _type, uint _level, uint damage, uint qaim) 
        ...
        returns(uint energy, uint graphene, uint metal, uint lock)
    {
        uint24[11] memory resourceCost = [0,120,252,522,1217,2616,5626,12096,26007,55916,120220];
        /*
            2584738 Level 11
            5557187 Level 12
            ----------------
        ...
        returns(uint)
    {
        uint8[4] memory movemmentPerMode = [4,6,3,0];
        uint ret;
        ret = (distance*40/movemmentPerMode[mode]);
        if (damage > 0) {
            ret = ((100 + damage) * ret) / 100;
        }
        if (qaim > 0) {
